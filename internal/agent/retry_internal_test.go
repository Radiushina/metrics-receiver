...
func TestRetry_PostMetric_TransportFailsThenOK(t *testing.T) {
	...
	err := PostGaugeMetric(client, "", srv.URL, "127.0.0.1", "x", models.Gauge, 1, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRetry_PostMetric_TransportAlwaysFails(t *testing.T) {
	...
	err := PostGaugeMetric(client, "", "http://127.0.0.1:1", "127.0.0.1", "x", models.Gauge, 1, nil)
	if err == nil {
...
func TestRetry_PostMetricsBatch_Status503ThenOK(t *testing.T) {
	...
	v := 1.0
	err := PostMetricsBatch(client, "", srv.URL, "127.0.0.1", []models.Metrics{
